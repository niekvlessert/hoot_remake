...
LibvgmYm2203::~LibvgmYm2203()
{
    if (chip_ != nullptr) {
        ym2203_shutdown(chip_);
        chip_ = nullptr;
    }
    if (ssg_ != nullptr) {
        ay8910_stop(ssg_);
...
bool LibvgmYm2203::initialize(uint32_t clock, uint32_t sample_rate)
{
    if (chip_ != nullptr) {
        ym2203_shutdown(chip_);
        chip_ = nullptr;
    ...
    chip_ = ym2203_init(this, clock, sample_rate, nullptr, nullptr);
    if (chip_ == nullptr) {
        return false;
    }
    if (ay8910_start(&ssg_, clock / 4, AYTYPE_YM2203, YM2149_PIN26_HIGH) == 0 || ssg_ == nullptr) {
        ym2203_shutdown(chip_);
        chip_ = nullptr;
        return false;
    }
    const ssg_callbacks callbacks = {
        &LibvgmYm2203::set_ssg_clock,
        &LibvgmYm2203::write_ssg,
        &LibvgmYm2203::read_ssg,
        &LibvgmYm2203::reset_ssg,
    };
    ym2203_link_ssg(chip_, &callbacks, this);
...
void LibvgmYm2203::write(uint8_t port, uint8_t data)
{
    if (chip_ != nullptr) {
        ym2203_write(chip_, static_cast<uint8_t>(port & 1), data);
    }
}

uint8_t LibvgmYm2203::read(uint8_t port)
{
    if (chip_ == nullptr) {
        return 0xff;
    }
...
void LibvgmYm2203::set_ssg_clock(void* param, uint32_t clock)
{
...
void LibvgmYm2203::write_ssg(void* param, uint8_t address, uint8_t data)
{
    auto* self = static_cast<LibvgmYm2203*>(param);
    if (self->ssg_ != nullptr) {
...
uint8_t LibvgmYm2203::read_ssg(void* param, uint8_t address)
{
    auto* self = static_cast<LibvgmYm2203*>(param);
    if (self->ssg_ == nullptr) {
        return 0xff;
...
void LibvgmYm2203::reset_ssg(void* param)
{
    auto* self = static_cast<LibvgmYm2203*>(param);
    if (self->ssg_ != nullptr) {

...
class MistyBlueDriver : public ssSoundDriver
{
public:
	enum {
		TYPE_GENERIC = 0,
	...
	static MistyBlueDriver *Instance(int _type = TYPE_GENERIC);
private:
	static MistyBlueDriver *m_Instance;
protected:
	MistyBlueDriver(int _type = TYPE_GENERIC);
public:
	virtual ~MistyBlueDriver();
...
MistyBlueDriver *MistyBlueDriver::m_Instance = NULL;

MistyBlueDriver *MistyBlueDriver::Instance(int _type)
{
	if (m_Instance == NULL) {
		m_Instance = new MistyBlueDriver(_type);
	}
	return m_Instance;
}

MistyBlueDriver::MistyBlueDriver(int _type)
{
	m_type = _type;
...
MistyBlueDriver::~MistyBlueDriver()
{
	if (m_YM2608) {
		delete m_YM2608;
	}
...
void MistyBlueDriver::WriteCtrl(WORD _adr, BYTE _data)
{
	ram[_adr] = _data;
}

BYTE MistyBlueDriver::ReadCtrl(WORD _adr)
{
	return 0xff;
}

void MistyBlueDriver::sWriteCtrl(WORD _adr, BYTE _data)
{
	MistyBlueDriver::m_Instance->WriteCtrl(_adr, _data);
}

BYTE MistyBlueDriver::sReadCtrl(WORD _adr)
{
	return MistyBlueDriver::m_Instance->ReadCtrl(_adr);
}

BYTE MistyBlueDriver::ReadPort(WORD _port)
{
...
void MistyBlueDriver::WritePort(WORD _port, BYTE _data)
{
	const BYTE port = _port;
	switch (port) {
	case 0x00:
		if (bgm_flag[_data]) {
			memcpy(&ram[0x9500], &bgm[BGM_SIZE * _data], BGM_SIZE);
			ram[LOAD_FLAG] = 0xff;
...
BYTE MistyBlueDriver::sReadPort(WORD _port)
{
	return MistyBlueDriver::m_Instance->ReadPort(_port);
}

void MistyBlueDriver::sWritePort(WORD _port, BYTE _data)
{
	MistyBlueDriver::m_Instance->WritePort(_port, _data);
}

void MistyBlueDriver::Execute(double _second)
{
	Lock();
...
bool MistyBlueDriver::Initialize(ssDriverConfig *_config)
{
...

	{
		...
		for (i = _config->romlist.begin(); i != _config->romlist.end(); i++) {
			if (i->type == "code") {
				...
				int p = i->offset;
				WORD *pcmadr = (WORD *)&ram[0x9400];
				zip.Load(i->filename,&pcmdata[pcmadr[p*4] * 4],
					(pcmadr[p*4+1] - pcmadr[p*4]) * 4 + 3);
			} else if (i->type == "bgm") {
...
void MistyBlueDriver::Interrupt(int _id)
{
	Lock();
	switch (_id) {
	case FM_TIMER_A:
...
bool MistyBlueDriver::Play(DWORD _code)
{
	Lock();
...
static ssSoundDriver *CreateDriver(const ssDriverConfig *_config)
{
	if (_config->driver_majortype == "mucom88") {
		if (_config->driver_subtype == "mistyblue") {
			return MistyBlueDriver::Instance(MistyBlueDriver::TYPE_MISTYBLUE);
		}
	}
	return NULL;
}
